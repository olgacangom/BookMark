...
describe('ClubsService', () => {
    ...
    describe('createClub', () => {
        it('should create and save club when creator exists', async () => {
            ...
            clubRepo.create!.mockReturnValue({ name: 'Club', creator, members: [creator] });
            clubRepo.save!.mockResolvedValue({ id: 'club1', name: 'Club' });

            const result = await service.createClub('user-1', { name: 'Club', description: 'Desc' });
        ...
        it('should throw if creator does not exist', async () => {
            ...
            await expect(service.createClub('user-1', { name: 'Club', description: 'Desc' })).rejects.toThrow(NotFoundException);
        });
    });

    describe('updateClub', () => {
        it('should update club when user is creator', async () => {
            const club = { id: 'club1', creator: { id: 'user-1' }, name: 'Old', description: 'Old', coverUrl: null };
            clubRepo.findOne!.mockResolvedValue(club);
            clubRepo.save!.mockResolvedValue({ ...club, name: 'New', description: 'New' });

            const result = await service.updateClub('user-1', 'club1', { name: 'New', description: 'New' });
        ...
        it('should throw if club not found', async () => {
            ...
            await expect(service.updateClub('user-1', 'club1', { name: 'New', description: 'New' })).rejects.toThrow(NotFoundException);
        });

        it('should throw if user is not creator', async () => {
            clubRepo.findOne!.mockResolvedValue({ creator: { id: 'user-2' } });
            await expect(service.updateClub('user-1', 'club1', { name: 'New', description: 'New' })).rejects.toThrow(ForbiddenException);
        });
    });
    ...
    describe('findOneClub', () => {
        ...
        it('throws if club not found', async () => {
            clubRepo.findOne!.mockResolvedValue(null);
            await expect(service.findOneClub('club1')).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('joinClub', () => {
        ...
        it('adds user to members and saves', async () => {
            ...
            clubRepo.findOne!.mockResolvedValue(club);
            userRepo.findOneBy!.mockResolvedValue({ id: 'user-1' });
            clubRepo.save!.mockResolvedValue({ id: 'club1', members: [{ id: 'user-1' }] });
        ...
        it('throws when club or user missing', async () => {
            clubRepo.findOne!.mockResolvedValue(null);
            userRepo.findOneBy!.mockResolvedValue(null);
            await expect(service.joinClub('user-1', 'club1')).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('createThread', () => {
        it('creates thread when club exists', async () => {
            ...
            threadRepo.create!.mockReturnValue({ title: 'Thread', club: { id: 'club1' } });
        ...
        it('throws if club not found', async () => {
            ...
            await expect(service.createThread('club1', { title: 'Thread' })).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('createPost', () => {
        it('creates post when thread and author exist', async () => {
            ...
            const result = await service.createPost('user-1', 'thread1', { content: 'hi', spoilerPage: 2 });
        ...
        it('throws when thread missing', async () => {
            ...
            userRepo.findOneBy!.mockResolvedValue({ id: 'user-1' });
            await expect(service.createPost('user-1', 'thread1', { content: 'hi', spoilerPage: 2 })).rejects.toThrow(NotFoundException);
        });
    ...
    describe('deleteClub', () => {
        ...
        it('throws if club not found', async () => {
            clubRepo.findOne!.mockResolvedValue(null);
            await expect(service.deleteClub('user-1', 'club1')).rejects.toThrow(NotFoundException);
        });

        it('throws if user is not creator', async () => {
            clubRepo.findOne!.mockResolvedValue({ creator: { id: 'user-2' } });
            await expect(service.deleteClub('user-1', 'club1')).rejects.toThrow(ForbiddenException);
        });
    });
    ...
    describe('findAllClubs', () => {
        it('returns all clubs with relations', async () => {
            ...
            expect(clubRepo.find).toHaveBeenCalledWith({ relations: ['creator', 'members'] });
