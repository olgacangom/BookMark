...
type MockAdminService = {
  getAllUsersWithBookCount: jest.Mock;
  getGlobalStats: jest.Mock;
  getMonthlyUserGrowth: jest.Mock;
  getMainStats: jest.Mock;
  findAllLibreros: jest.Mock;
  approveLibrero: jest.Mock;
  rejectLibrero: jest.Mock;
  toggleUserStatus: jest.Mock;
};

type MockLibrerosService = {
  getMyInventory: jest.Mock;
  addToInventory: jest.Mock;
  updateInventoryItem: jest.Mock;
  removeFromInventory: jest.Mock;
  getStats: jest.Mock;
  updateProfile: jest.Mock;
  findStoresByBook: jest.Mock;
  getMyEvents: jest.Mock;
  createEvent: jest.Mock;
  updateEvent: jest.Mock;
  deleteEvent: jest.Mock;
  getAllFutureEvents: jest.Mock;
  joinEvent: jest.Mock;
  getEventAttendees: jest.Mock;
};
...
describe('Roles Controllers', () => {
  ...
  let librerosService: MockLibrerosService;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: LibrerosService, useValue: librerosService },
      ],
    }).compile();
  ...
  it('should get all libreros', async () => {
    await adminController.getAllLibreros();
    expect(adminService.findAllLibreros).toHaveBeenCalled();
  });

  it('should verify librero', async () => {
    await adminController.verifyLibrero('123');
    expect(adminService.approveLibrero).toHaveBeenCalledWith('123');
  });

  it('should reject librero', async () => {
    await adminController.rejectLibrero('123');
    expect(adminService.rejectLibrero).toHaveBeenCalledWith('123');
  });

  it('should toggle user status', async () => {
    await adminController.toggleUserStatus('123');
  ...
  it('should add book to inventory', async () => {
    ...
    expect(librerosService.addToInventory).toHaveBeenCalledWith('u1', 'b1', {
      price: 10,
      inStock: true,
    });
  });
  ...
  it('should update inventory item', async () => {
    await libreroController.updateInventory(
      'inv1',
  ...
  it('should create event', async () => {
    await libreroController.createEvent(req, {
      title: 'event',
    } as LibraryEvent);
  ...
  it('should update event', async () => {
    ...
    expect(librerosService.updateEvent).toHaveBeenCalledWith('u1', 'e1', {
      title: 'x',
    });
  });
