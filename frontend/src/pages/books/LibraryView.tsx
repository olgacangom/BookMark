...
import {
    ...
    BookOpen, MessageSquare, SortAsc, StickyNote
} from 'lucide-react';
        ...
        <div className="min-h-screen bg-[#F0F9F9] font-sans text-slate-900 pb-20">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
                        {filteredBooks.map((book) => (
                            <BookCard
                                ...
                                onOpenNotes={(book) => { setSelectedBook(book); setIsNotesOpen(true); }}
                            />
                        ))}
                    </div>
                ) : (
            ...
            {isNotesOpen && selectedBook && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-in fade-in duration-300">
                    <div className="bg-[#F0F9F9] rounded-[3rem] w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col shadow-2xl border-8 border-white">
                        <div className="p-6 bg-white border-b border-slate-100 flex justify-between items-center">
                            <div>
                                <h3 className="font-black text-slate-900 uppercase text-lg leading-tight">{selectedBook.title}</h3>
                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Diario de Lectura</p>
                            </div>
                            <button onClick={() => setIsNotesOpen(false)} className="text-slate-300 hover:text-slate-900 transition-colors">✕</button>
                        </div>
            ...
            <div className="relative mb-3 aspect-[2/3] rounded-2xl overflow-hidden shadow-lg group-hover:shadow-2xl transition-all bg-slate-100">
                ...
                <div className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-center p-4 gap-2">
                    ...
                    <button 
                        ...
                        className="w-full bg-teal-600 text-white py-2 rounded-xl text-[10px] font-bold uppercase tracking-tight hover:bg-teal-700 transition-all flex items-center justify-center gap-2"
                    >
